...
func (c *LogCacheCLI) Run(conn plugin.CliConnection, args []string) {
	...
	switch args[0] {
	case "query":
		command.Query(context.Background(), conn, args[1:], http.DefaultClient)
	case "tail":
...
ENVIRONMENT VARIABLES:
   ...
   LOG_CACHE_SKIP_AUTH  Set to 'true' to disable CF authentication.`,
					Options: map[string]string{
						"--time":  "Effective time for query execution of an instant query. Cannot be used with --start, --end, or --step. Must be a Unix Timestamp or RFC3339 DateTime.",
						"--start": "Start time for a range query. Must be a Unix Timestamp or RFC3339 DateTime.",
						"--end":   "End time for a range query. Must be a Unix Timestamp or RFC3339 DateTime.",
						"--step":  "Step interval for a range query. Must be a PromQL Time Duration.",
					},
				},
			},
		},
